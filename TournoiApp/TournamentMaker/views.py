...
def dashboard(request, tournament_slug):
    ...
    if final:
        final_winner = final.get_match_winner()
        if final_winner:
            winner = final_winner
            finalist = final.team_b if final_winner == final.team_a else final.team_a

    # Récupération de la petite finale
    petite_finale = Match.objects.filter(
        tournament=tournoi,
        phase='third_place',
        team_a__isnull=False,
        team_b__isnull=False,
    ...
    if petite_finale:
        third_place = petite_finale.get_match_winner()
